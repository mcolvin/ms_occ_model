model<- function ()
	...
	for(i in 1:nfish) 
		...
		for(j in 1:no.orgs)
			...
			for(h in 1:3) 
				{
				expected[i,j,h] <- theta2[i,j,h,lat.org2[i,j]]
				resid[i,j,h] <- y[i,j,h] - expected[i,j,h]
				residSquared[i,j,h] <- pow(resid[i,j,h], 2)
				
				y.sim[i,j,h] ~ dbin(theta2[i,j,h,lat.org2[i,j]], 1)
				resid.sim[i,j,h] <- y.sim[i,j,h] - expected[i,j,h]
				residSquared.sim[i,j,h] <- pow(resid.sim[i,j,h], 2)
				} # end h
			} # end j
		}# end i
	rss <- sum(residSquared[,,])
	rss_sim <- sum(residSquared.sim[,,])
	## END GOODNESS OF FIT
	
# derived parameters
for(i in 1:5)
	{
	prev_org[i]<- sum(lat.org[,i])/nfish
	}
...
for(i in 1:7)
	{
	prev_organ[i]<- sum(lat.org[,i])/nfish
	}
logit(pred.fish) <- alpha0	## predict fish occupancy
for(o in 1:no.orgs)
	{
...
model_AE_M01<- function ()
	...
	for(i in 1:nfish) 
		...
		for(h in 1:3) 
			{
			expected[i,h] <- theta2[i,h,lat.fish2[i]]
			resid[i,h] <- y[i,h] - expected[i,h]
			residSquared[i,h] <- pow(resid[i,h], 2)
			
			y.sim[i,h] ~ dbin(theta2[i,h,lat.fish2[i]], 1)
			resid.sim[i,h] <- y.sim[i,h] - expected[i,h]
			residSquared.sim[i,h] <- pow(resid.sim[i,h], 2)
			} # end h
		}# end i
	rss <- sum(residSquared[,])
	rss_sim <- sum(residSquared.sim[,])
